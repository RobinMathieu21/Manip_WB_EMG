...
function [EMG_phasique , tonic_meanstd, Vmean_RMS_R, Vmean_RMS_B, norm_tonic_R_moy, norm_tonic_B_moy] = compute_emg2_TonicNew_WB(RMS, RMS_R_norm2, RMS_B_norm2, ...
 emg_frequency, tonic_debut, tonic_fin, Idx, limite_en_temps, signal_mvt_lent_R_complet, signal_mvt_lent_B_complet, Nb_emgs, Phasic_lent, ordre_filtre_phasic, passebas_filtre_phasic)   
...
%         norm_tonic_B_moy(anticip-EMD+1:profil_sizes_B_mean(1, i)-anticip-EMD, i) = normalize2(tonics_B_meanstd(1:2, i), 'PCHIP', profil_sizes_B_mean(1, i)-2*anticip);
        norm_tonic_R_moy(anticip+1:profil_sizes_R_mean(1, i)-anticip, i) = normalize2(signal_mvt_lent_R, 'PCHIP', profil_sizes_R_mean(1, i)-2*anticip);
        norm_tonic_B_moy(anticip+1:profil_sizes_B_mean(1, i)-anticip, i) = normalize2(signal_mvt_lent_B, 'PCHIP', profil_sizes_B_mean(1, i)-2*anticip);
        
        % ON PREND EN COMPTE LES SIGNAUX DES MVTS rapides ENTRE '0' et
        % ANTICIP - EMD
        for g = 1:anticip-EMD
%             norm_tonic_R_moy(g, i) = norm_tonic_R_moy(anticip-EMD+1, i);
%             norm_tonic_B_moy(g, i) = norm_tonic_B_moy(anticip-EMD+1, i);
            norm_tonic_R_moy(g, i) = tonics_R_meanstd(1, i);
            norm_tonic_B_moy(g, i) = tonics_B_meanstd(1, i);
        end
        transition_R_deb(1,1)=tonics_R_meanstd(1, i); transition_R_deb(2,1)=norm_tonic_R_moy(anticip+1,i);
        transition_B_deb(1,1)=tonics_B_meanstd(1, i); transition_B_deb(2,1)=norm_tonic_B_moy(anticip+1,i);
        norm_tonic_R_moy(anticip-EMD:anticip, i) = normalize2(transition_R_deb, 'PCHIP', EMD+1);
        norm_tonic_B_moy(anticip-EMD:anticip, i) = normalize2(transition_B_deb, 'PCHIP', EMD+1);
        
        % ON PREND EN COMPTE LES SIGNAUX DES MVTS rapides ENTRE 'FULL LENGTH - ANTICIP +EMD' et
        % FULL LENGTH
        for g = 1:anticip-EMD
...
%             norm_tonic_B_moy(profil_sizes_B_mean(1, i)-g+1, i) = norm_tonic_B_moy(profil_sizes_B_mean(1, i)-anticip-EMD, i);
            norm_tonic_R_moy(profil_sizes_R_mean(1, i)-g+1, i) = tonics_R_meanstd(2, i);
            norm_tonic_B_moy(profil_sizes_B_mean(1, i)-g+1, i) = tonics_B_meanstd(2, i);
        end
        
        transition_R_fin(2,1)=tonics_R_meanstd(2, i); transition_R_fin(1,1)=norm_tonic_R_moy(profil_sizes_R_mean(1, i)-anticip-1,i);
        transition_B_fin(2,1)=tonics_B_meanstd(2, i); transition_B_fin(1,1)=norm_tonic_B_moy(profil_sizes_B_mean(1, i)-anticip-1,i);
        norm_tonic_R_moy(profil_sizes_R_mean(1, i)-anticip:profil_sizes_R_mean(1, i)-anticip+EMD, i) = normalize2(transition_R_fin, 'PCHIP', EMD+1);
        norm_tonic_B_moy(profil_sizes_B_mean(1, i)-anticip:profil_sizes_B_mean(1, i)-anticip+EMD, i) = normalize2(transition_B_fin, 'PCHIP', EMD+1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [~, nb_col_rms] = size(emg_phasique_R_avantNorm_FILT);
    i = 1;
    j = 1;
    ...
    %% Calculs quantif desac pour le mvt lever
        ...
        for f = 1 : length(emg_phasique_R_avantNorm_FILT(:, j)) % Une boucle pour tester toutes les valeurs du phasic
            
            if emg_phasique_R_avantNorm_FILT(f, j) < -2*mean(tonics_R_meanstd(3, :)) % Si la valeur est inf à zero indic est incrementé
               indic = indic + 1 ;
            else   % Sinon
        ...
        [Pmin, indice] = min(emg_phasique_R_avantNorm_FILT(:, j));
        if Pmin > 0
            Pmin =0;
        end
        amplitude = Pmin * 100 / norm_tonic_R_moy(indice,j);
        ...
        Limite_basse_detection = round(emg_frequency * limite_en_temps); %Limite d'image à atteindre pour considérer la phase négative 40ms, arrondi à l'image près
        for f = 1 : length(emg_phasique_B_avantNorm_FILT(:, j))% Une boucle pour tester toutes les valeurs du phasic
            
            if emg_phasique_B_avantNorm_FILT(f, j) < -2*mean(tonics_R_meanstd(3, :)) % Si la valeur est inf à zero indic est incrementé
               indic = indic + 1 ;
            else   % Sinon
                if Limite_atteinte % Soit la limite avait déjà été atteinte (la phase doit donc être comptée)
        ...
        % Calcul de l'amplitude max de négativité
        [Pmin, indice] = min(emg_phasique_B_avantNorm_FILT(:, j));
        if Pmin > 0
            Pmin =0;
        end
        amplitude2 = Pmin * 100 / norm_tonic_B_moy(indice,j);
    ...
    %% Enregistrement des données
        ...
        EMG_phasique.QuantifDesac(j, (WW-1)*12+1) = (100*compteur/length(emg_phasique_R_avantNorm_FILT(:, j)));  % Pour l'avoir en temps
        EMG_phasique.QuantifDesac(j, (WW-1)*12+2) = (100*compteur2/length(emg_phasique_B_avantNorm_FILT(:, j))); % Pour l'avoir en temps  
    ...
    EMG_phasique.TonicSoustrait.R(:,(WW-1)*nb/2+1:WW*nb/2) = norm_tonic_R_moy;
    EMG_phasique.TonicSoustrait.B(:,(WW-1)*nb/2+1:WW*nb/2) = norm_tonic_B_moy;
